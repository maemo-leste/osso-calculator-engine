...
copyvalue(VALUE *oldvp, VALUE *newvp)
{
	...
	if (oldvp->v_type >= 0) {
		...
		case V_HASH:
			...
			break;
		case V_BLOCK:
			/* newvp->v_block = blk_copy(oldvp->v_block); -TL*/
			break;
		case V_OCTET:
			newvp->v_type = V_NUM;
...
powervalue(VALUE *v1, VALUE *v2, VALUE *v3, VALUE *vres)
{
  ...
  {//fractional is 0, we can use the old power func
    vres->v_num=qpowi(v1->v_num,v2->v_num);
    vres->v_type = V_NUM;
    freevalue(&fracnum);
    return;
  }
...
comparevalue(VALUE *v1, VALUE *v2)
{
	...
	switch (v1->v_type) {
		...
		case V_HASH:
			...
			break;
		case V_BLOCK:
			/* r = blk_cmp(v1->v_block, v2->v_block); -TL*/
			break;
		case V_OCTET:
