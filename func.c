...
static int malloced_putenv(char *str); -TL*/
...
f_tan(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp1.v_type = V_COM;
			tmp1.v_com = mycsin(vals[0]->v_com, err);
			tmp2.v_type = V_COM;
			tmp2.v_com = myccos(vals[0]->v_com, err);
...
f_sec(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp.v_type = V_COM;
			tmp.v_com = myccos(vals[0]->v_com, err);
...
f_cot(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp1.v_type = V_COM;
			tmp1.v_com = myccos(vals[0]->v_com, err);
			tmp2.v_type = V_COM;
			tmp2.v_com = mycsin(vals[0]->v_com, err);
			divvalue(&tmp1, &tmp2, &result);
			comfree(tmp1.v_com);
			comfree(tmp2.v_com);
			break;
...
f_csc(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp.v_type = V_COM;
			tmp.v_com = mycsin(vals[0]->v_com, err);
			invertvalue(&tmp, &result);
			comfree(tmp.v_com);
			break;
		default:
...
f_tanh(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp1.v_type = V_COM;
			tmp1.v_com = mycsinh(vals[0]->v_com, err);
			tmp2.v_type = V_COM;
			tmp2.v_com = myccosh(vals[0]->v_com, err);
			divvalue(&tmp1, &tmp2, &result);
			comfree(tmp1.v_com);
			comfree(tmp2.v_com);
			break;
...
f_coth(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp1.v_type = V_COM;
			tmp1.v_com = myccosh(vals[0]->v_com, err);
			tmp2.v_type = V_COM;
			tmp2.v_com = mycsinh(vals[0]->v_com, err);
			divvalue(&tmp1, &tmp2, &result);
			comfree(tmp1.v_com);
			comfree(tmp2.v_com);
			break;
...
f_sech(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp.v_type = V_COM;
			tmp.v_com = myccosh(vals[0]->v_com, err);
			invertvalue(&tmp, &result);
			comfree(tmp.v_com);
			break;
		default:
...
f_csch(int count, VALUE **vals)
{
	...
	switch (vals[0]->v_type) {
		...
		case V_COM:
			tmp.v_type = V_COM;
			tmp.v_com = mycsinh(vals[0]->v_com, err);
			invertvalue(&tmp, &result);
			comfree(tmp.v_com);
			break;
		default:
	...
	switch (vals[0]->v_type) {
		case V_NUM:
			...
			if (result.v_num == NULL) {
				...
				result.v_type = V_COM;
				result.v_com = mycasin(tmp, err);
				comfree(tmp);
			}
			break;
		case V_COM:
			result.v_com = mycasin(vals[0]->v_com, err);
			result.v_type = V_COM;
			break;
		default:
	...
	switch (vals[0]->v_type) {
		case V_NUM:
			...
			if (result.v_num == NULL) {
				...
				tmp->real = qlink(vals[0]->v_num);
				result.v_type = V_COM;
				result.v_com = mycacos(tmp, err);
				comfree(tmp);
			}
			break;
		case V_COM:
			result.v_com = mycacos(vals[0]->v_com, err);
			result.v_type = V_COM;
			break;
		default:
	...
	switch (vals[0]->v_type) {
		case V_NUM:
			...
			if (result.v_num == NULL) {
				...
				qfree(tmp->real);
				tmp->real = qlink(vals[0]->v_num);
				result.v_com = mycacosh(tmp, err);
				result.v_type = V_COM;
				comfree(tmp);
			}
			break;
		case V_COM:
			result.v_com = mycacosh(vals[0]->v_com, err);
			result.v_type = V_COM;
			break;
		default:
	...
	switch (vals[0]->v_type) {
		case V_NUM:
			...
			if (result.v_num == NULL) {
				...
				qfree(tmp->real);
				tmp->real = qlink(vals[0]->v_num);
				result.v_com = mycatanh(tmp, err);
				result.v_type = V_COM;
				comfree(tmp);
			}
			break;
		case V_COM:
			result.v_com = mycatanh(vals[0]->v_com, err);
			result.v_type = V_COM;
			break;
		default:
...
static CONST struct builtin builtins[] = {
	...
	{"root", 2, 3, 0, OP_NOP, 0, f_root,
	 "value a taken to the b'th root within accuracy c"}, -TL */
	...
	{"sort", 1, 1, 0, OP_NOP, 0, f_sort,
	 "sort a copy of a matrix or list"}, -TL*/
