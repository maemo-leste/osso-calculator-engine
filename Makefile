...
#CALC_SHAREDIR= /dev/env/DJDIR/share/calc
#CALC_SHAREDIR= /usr/share/calc
CALC_SHAREDIR= /usr/share/osso_calculator_engine

#CALC_INCDIR= /usr/local/include/calc
#CALC_INCDIR= /dev/env/DJDIR/include/calc
#CALC_INCDIR= ${INCDIR}/calc
CALC_INCDIR= ${INCDIR}/osso_calculator_engine
...
BINARY=osso_calculator_engine
LIBRARY=lib_osso_calculator_engine

# For dist building
PACKAGE = "osso-calculator-engine"
VERSION = "1.0.38"
GZIP_ENV = --best
SUBDIRS = 
...
DIST_COMMON = CHANGES BUGS COPYING COPYING-LGPL HOWTO.INSTALL LIBRARY README \
	calcerr.tbl calcerr_c.awk calcerr_c.sed calcerr_h.awk calcerr_h.sed calc.spec.in \
	check.awk Makefile

DIST_SOURCES = addop.c const.c have_getsid.c have_ustat.c no_implicit.c seed.c zmod.c \
	endian.c have_gettime.c have_varvs.c obj.c shs.c zmul.c assocfunc.c \
	have_memmv.c opcodes.c size.c zprime.c fposval.c have_newstr.c input.c \
	poly.c string.c zrand.c func.c have_offscl.c jump.c prime.c symbol.c zrandom.c \
	byteswap.c hash.c have_posscl.c label.c qfunc.c token.c calc.c have_const.c \
	have_rusage.c lib_calc.c qio.c value.c codegen.c have_fpos.c have_stdvs.c listfunc.c \
	qmath.c version.c comfunc.c have_fpos_pos.c have_strdup.c longbits.c qmod.c zfunc.c \
	commath.c have_getpgid.c have_uid_t.c matfunc.c qtrans.c zio.c config.c have_getprid.c \
	have_unused.c math_error.c quickhash.c zmath.c \
	alloc.h byteswap.h config.h hash.h label.h opcodes.h shs.h token.h zmath.h \
	calc.h math_error.h prime.h string.h value.h zrand.h block.h cmath.h \
	func.h jump.h nametype.h qmath.h symbol.h win32dll.h zrandom.h
...
CCMISC=
#
#CFLAGS= -DCALC_SRC ${CCWARN} ${CCOPT} ${CCMISC} -O1
...
#
LIBSRC= addop.c assocfunc.c byteswap.c \
	codegen.c comfunc.c commath.c config.c const.c \
	func.c hash.c input.c jump.c label.c \
	lib_calc.c listfunc.c matfunc.c math_error.c \
	obj.c opcodes.c poly.c prime.c qfunc.c qio.c \
	qmath.c qmod.c qtrans.c quickhash.c seed.c shs.c size.c \
	string.c symbol.c token.c value.c version.c zfunc.c zio.c \
	zmath.c zmod.c zmul.c zprime.c zrand.c zrandom.c
 	#blkcpy.c block.c custom.c file.c help.c hist.c lib_util.c md5.c pix.c shs1.c
...
#
LIBOBJS= addop.o assocfunc.o byteswap.o calcerr.o \
	codegen.o comfunc.o commath.o config.o const.o \
	func.o hash.o input.o jump.o label.o \
	lib_calc.o listfunc.o matfunc.o math_error.o \
	obj.o opcodes.o poly.o prime.o qfunc.o qio.o \
	qmath.o qmod.o qtrans.o quickhash.o seed.o shs.o size.o \
	string.o symbol.o token.o value.o version.o zfunc.o zio.o \
	zmath.o zmod.o zmul.o zprime.o zrand.o zrandom.o
	#blkcpy.o block.o custom.o file.o help.o hist.o lib_util.o md5.o pix.o shs1.o
...
#
LIB_H_SRC= alloc.h block.h byteswap.h calc.h cmath.h \
	config.h func.h hash.h jump.h \
	label.h math_error.h nametype.h \
	opcodes.h prime.h qmath.h shs.h string.h \
	symbol.h token.h value.h zmath.h zrand.h zrandom.h
	#blkcpy.h custom.h file.h hist.h lib_util.h md5.h shs1.h win32dll.h

# we build these .h files during the make
#
BUILD_H_SRC= args.h calcerr.h conf.h endian_calc.h \
...
#
UTIL_C_SRC= endian.c longbits.c have_newstr.c have_uid_t.c \
...
#
UTIL_PROGS= fposval${EXT} have_uid_t${EXT} have_const${EXT} \
...
DISTLIST= ${C_SRC} ${H_SRC} ${MAKE_FILE} BUGS CHANGES LIBRARY README \
	  README.WINDOWS HOWTO.INSTALL ${UTIL_MISC_SRC} ${LICENSE} \
	  calc.spec.in
	#calc.man rpm.mk
...
${BINARY}${EXT}: .hsrc ${CALC_LIBS} ${CALCOBJS}
	${RM} -f $@
	${CC} ${LDFLAGS} ${CALCOBJS} ${CALC_LIBS} ${LD_DEBUG} \
	      ${READLINE_LIB} -o $@

${LIBRARY}.a: ${LIBOBJS} ${MAKE_FILE}
	-rm -f ${LIBRARY}.a
	ar qc ${LIBRARY}.a ${LIBOBJS}
	${RANLIB} ${LIBRARY}.a
	${CHMOD} 0644 ${LIBRARY}.a

#${BINARY}.1: ${BINARY}.man ${MAKE_FILE}
#	-rm -f ${BINARY}.1
...
#	       -e 's,$${CALCRC},${CALCRC},g' < ${BINARY}.man > ${BINARY}.1
...
cvd:
	${CALC_ENV} cvd ./${BINARY}${EXT}

dbx:
	${CALC_ENV} dbx ./${BINARY}${EXT}

gdb:
	${CALC_ENV} gdb ./${BINARY}${EXT}
...
inst_files: ${MAKE_FILE} ver_calc${EXT}
	${V} echo '=-=-=-=-= start of $@ rule =-=-=-=-='
	${Q}rm -f inst_files
	${Q}echo ${BINDIR}/${BINARY}${EXT} > inst_files
#	${Q}cd help; LANG=C \
...
#	    ${GREP} '__file__..' | ${SED} -e s'/.*__file__ //' >> ../inst_files
	${Q}echo ${LIBDIR}/${LIBRARY}.a >> inst_files
...
#	-rmdir /usr/local/lib/calc/custom
	-rmdir /usr/local/lib/${BINARY}
	-rmdir /usr/local/include/${BINARY}
...
clobber:
	...
	-rm -f ${BUILD_H_SRC}
	-rm -f ${BUILD_C_SRC}
	-rm -f ${BINARY}${EXT} *_pure_*.[oa]
	-rm -f ${LIBRARY}.a *.pure_hardlink
	-rm -f ${BINARY}.1 *.pure_linkinfo
	-rm -f *.u
	-rm -f ${BINARY}.pixie ${BINARY}.rf ${BINARY}.Counts ${BINARY}.cord
	-rm -rf gen_h skel Makefile.bak tmp.patch
	-rm -f ${BINARY}.spec inst_files rpm.mk.patch tmp
...
#	-rm -f endian.h stdarg.h libcalcerr.a cal/obj help/obj
	-rm -f endian.h stdarg.h
	-rm -f have_vs.c std_arg.h try_stdarg.c fnvhash.c ${BINARY}.spec
#	-rm -rf win32
	${V} echo '=-=-=-=-= end of $@ rule =-=-=-=-='

install: ${BINARY} ${LIBRARY}.a
	${V} echo '=-=-=-=-= start of $@ rule =-=-=-=-='
	-${Q}if [ ! -z "$T" ]; then \
	    if [ ! -d $T ]; then \
...
#	fi
	...
	-${Q}if ${CMP} -s ${BINARY}${EXT} $T${BINDIR}/${BINARY}${EXT}; then \
	    true; \
	else \
	    rm -f $T${BINDIR}/${BINARY}.new${EXT}; \
	    cp -f ${BINARY}${EXT} $T${BINDIR}/${BINARY}.new${EXT}; \
	    ${CHMOD} 0555 $T${BINDIR}/${BINARY}.new${EXT}; \
	    mv -f $T${BINDIR}/${BINARY}.new${EXT} $T${BINDIR}/${BINARY}${EXT}; \
	    echo "installed $T${BINDIR}/${BINARY}${EXT}"; \
	fi
#	${V} echo '=-=-=-=-= Invoking $@ rule for help =-=-=-=-='
...
#	${V} echo '=-=-=-=-= Back to the main Makefile for $@ rule =-=-=-=-='
	-${Q}if ${CMP} -s ${LIBRARY}.a $T${LIBDIR}/${LIBRARY}.a; then \
		true; \
	else \
		rm -f $T${LIBDIR}/${LIBRARY}.a.new; \
		cp -f ${LIBRARY}.a $T${LIBDIR}/${LIBRARY}.a.new; \
		mv -f $T${LIBDIR}/${LIBRARY}.a.new $T${LIBDIR}/${LIBRARY}.a; \
		${RANLIB} $T${LIBDIR}/${LIBRARY}.a; \
		${CHMOD} 0444 $T${LIBDIR}/${LIBRARY}.a; \
		echo "installed $T${LIBDIR}/${LIBRARY}.a"; \
	fi
#	-${Q}for i in ${LIB_H_SRC} ${BUILD_H_SRC} /dev/null; do \
#	    if [ "$$i" = "/dev/null" ]; then \
#		continue; \
#	    fi; \
#	    rm -f tmp; \
#	    ${SED} -e 's/^\(#[ 	]*include[ 	][ 	]*\)"/\1"${BINARY}\//' $$i > tmp; \
...
#	    true; \
#	else \
#	    if ${CMP} -s ${BINARY}.1 $T${MANDIR}/${BINARY}.${MANEXT}; then \
#		true; \
#	    else \
#		rm -f $T${MANDIR}/${BINARY}.${MANEXT}.new; \
#		cp -f ${BINARY}.1 $T${MANDIR}/${BINARY}.${MANEXT}.new; \
#		${CHMOD} 0444 $T${MANDIR}/${BINARY}.${MANEXT}.new; \
#		mv -f $T${MANDIR}/${BINARY}.${MANEXT}.new \
#		      $T${MANDIR}/${BINARY}.${MANEXT}; \
#		echo "installed $T${MANDIR}/${BINARY}.${MANEXT}"; \
#	    fi; \
#	fi
#	-${Q}if [ -z "${CATDIR}" ]; then \
#	    true; \
...
#	    else \
#		if [ -z "${NROFF}" ]; then \
#		    echo "${MANMAKE} ${BINARY}.1 $T${CATDIR}"; \
#		    ${MANMAKE} ${BINARY}.1 $T${CATDIR}; \
#		else \
#		    rm -f $T${CATDIR}/${BINARY}.${CATEXT}.new; \
#		    ${NROFF} ${NROFF_ARG} ${BINARY}.1 > \
#			     $T${CATDIR}/${BINARY}.${CATEXT}.new; \
#		    ${CHMOD} ${MANMODE} $T${MANDIR}/${BINARY}.${CATEXT}.new; \
#		    mv -f $T${CATDIR}/${BINARY}.${CATEXT}.new \
#			  $T${CATDIR}/${BINARY}.${CATEXT}; \
#		    echo "installed $T${CATDIR}/${BINARY}.${CATEXT}"; \
#		fi; \
#	    fi; \
#	fi
	${V} echo '=-=-=-=-= end of $@ rule =-=-=-=-='
